...
{
    public class WhenAttackingAndNotSunkingAShip : GivenBoard80X40
    {
        private IShip _ship;
        ...
        [Fact]
        public void Should_Only_Hit_The_Ship_Twice()
        {
            Mock.Get(_ship)
                .Verify(s => s.Hit(It.IsAny<Point>()), Times.Exactly(2));
            Assert.All(_locationsToAttack,
                point => { Assert.Contains(point, _ship.Coordinates.Select(s => s.Location)); });
        }
